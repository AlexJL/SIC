...
function onDeviceReady() {
	...
	$("#app-status-ul").append('<li>Device ok. Evento ativado.</li>');
			...
			pushNotification.register(successHandler, errorHandler, {"senderID":"137538505037","ecb":"onNotification"});		// required!
					} else {
            ...
            function onNotification(e) {
                ...
                switch( e.event )
                {
					...
					if ( e.regid.length > 0 )
					{
                        ...
                        $.ajax({
                            type: "POST",
                            url: "http://192.168.22.10/push/guardar.php",
                            data: "key=" + e.regid,
                            cache: false,
                            dataType: "text",
                    ...
                    case 'message':
                    	...
                    	if (e.foreground)
                    	{
							...
							my_media.play();
                            if(e.payload.title == "NOTICIA"){
                                alert("Se ha agregado una nueva noticia")
                            }else if(e.payload.title == "RECIBO"){
                                alert("Se ha agregado un nuevo recibo")
                            }
						}
						else
						...
						$("#app-status-ul").append('<li>TITLE -> MSG: ' + e.payload.title + '</li>');	
